...
export default function PackageCustomerDetailsModal({ isOpen, onClose, package: pkg, selectedDate, guests, totalAmount }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // For packages, we need to create a booking for the package
      // Note: Your current Booking model expects a service. This needs to be handled.
      // For now, we'll use the first service in the package as a placeholder.
      // You may need to modify the backend to support package bookings.
      
      const firstService = pkg.services && pkg.services[0];
      if (!firstService) {
        setError('This package has no services. Cannot book.');
        setIsLoading(false);
        return;
      }
      ...
      const bookingData = {
        ...
        service: firstService.id,  // Placeholder - using first service
        date: selectedDate,
        time: '10:00:00',
        guests: guests,
